...
namespace Sango.Core
{
    public class SkillTestSystem
    {
        ...
        public void TestAllSkills()
        {
            ...
            Sango.Log.Info("技能测试完成!");
        }

        private void TestSkillVisualSystem()
        {
            Sango.Log.Info("测试技能表现系统...");
            // 测试技能视觉效果的初始化
            ...
            Sango.Log.Info($"默认视觉效果器: {defaultVisualizer != null}");
            Sango.Log.Info($"远程视觉效果器: {rangeVisualizer != null}");
            Sango.Log.Info($"近战视觉效果器: {meleeVisualizer != null}");
            Sango.Log.Info($"计策视觉效果器: {strategyVisualizer != null}");
        }

        private void TestOffsetSkillSystem()
        {
            Sango.Log.Info("测试位移技能系统...");
            // 测试位移类型的数量
            int offsetTypeCount = System.Enum.GetValues(typeof(SkillCellOffsetType)).Length;
            Sango.Log.Info($"位移类型数量: {offsetTypeCount}");
            ...
            Sango.Log.Info($"包含MasterRandom: {hasMasterRandom}");
            Sango.Log.Info($"包含TargetRandom: {hasTargetRandom}");
            Sango.Log.Info($"包含Master指定位置: {hasMaster指定位置}");
            Sango.Log.Info($"包含Target指定位置: {hasTarget指定位置}");
        }

        private void TestRangeSkillSystem()
        {
            Sango.Log.Info("测试范围技能系统...");
            // 测试范围类型的数量
            int rangeTypeCount = System.Enum.GetValues(typeof(SkillAttackOffsetType)).Length;
            Sango.Log.Info($"范围类型数量: {rangeTypeCount}");
            ...
            Sango.Log.Info($"包含Fan: {hasFan}");
            Sango.Log.Info($"包含Rectangle: {hasRectangle}");
            Sango.Log.Info($"包含Cross: {hasCross}");
            Sango.Log.Info($"包含Square: {hasSquare}");
            Sango.Log.Info($"包含Diamond: {hasDiamond}");
        }

        private void TestBuffSystem()
        {
            Sango.Log.Info("测试BUFF系统...");
            // 测试BUFF效果的初始化
            BuffEffect.Init();
            ...
            Sango.Log.Info($"中毒效果: {poisonEffect != null}");
            Sango.Log.Info($"燃烧效果: {burnEffect != null}");
            Sango.Log.Info($"冰冻效果: {freezeEffect != null}");
            Sango.Log.Info($"沉默效果: {silenceEffect != null}");
            Sango.Log.Info($"无敌效果: {invincibleEffect != null}");
            Sango.Log.Info($"护盾效果: {shieldEffect != null}");
        }

        private void TestSkillAISystem()
        {
            Sango.Log.Info("测试技能AI系统...");
            // 测试AI系统是否能够正常工作
            // 这里可以添加具体的AI测试逻辑
            Sango.Log.Info("技能AI系统测试完成");
        }

        private void TestSkillConfigSystem()
        {
            Sango.Log.Info("测试技能配置系统...");
            // 测试技能配置管理器的初始化
            ...
            Sango.Log.Info($"技能配置数量: {skillConfigs.Count}");
            Sango.Log.Info($"BUFF配置数量: {buffConfigs.Count}");
        }

        public void TestSkillBalance()
        {
            Sango.Log.Info("开始技能平衡测试...");
            ...
            Sango.Log.Info("技能平衡测试完成!");
        }

        private void TestSkillDamageBalance()
        {
            Sango.Log.Info("测试技能伤害平衡...");
            // 这里可以添加具体的伤害平衡测试逻辑
            Sango.Log.Info("技能伤害平衡测试完成");
        }

        private void TestSkillCostBalance()
        {
            Sango.Log.Info("测试技能消耗平衡...");
            // 这里可以添加具体的消耗平衡测试逻辑
            Sango.Log.Info("技能消耗平衡测试完成");
        }

        private void TestSkillRangeBalance()
        {
            Sango.Log.Info("测试技能范围平衡...");
            // 这里可以添加具体的范围平衡测试逻辑
            Sango.Log.Info("技能范围平衡测试完成");
        }
    }
}
